...
export default function GeneratorTab({ onStatus, onPasswordGenerated }) {
  ...
  useEffect(() => {
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [length, useLower, useUpper, useDigits, useSymbols, excludeAmbiguous, noConsecutiveRepeats, noRepeatedChars]);
      ...
      <Panel className="p-5">
        <div className="mb-4">
          <div className="flex items-center justify-between mb-2">
            ...
            <span className="font-mono text-brass-bright text-sm">{length}</span>
          </div>
          <input
            type="range"
            min={4}
        ...
        <div className="flex items-center gap-3 mt-5 pt-4 border-t border-line flex-wrap">
          <Button onClick={() => regenerate(true)}>Generate password</Button>
          <div className="flex items-center gap-2 ml-auto">
            ...
            <Button variant="ghost" onClick={generateBatch}>Generate batch</Button>
          </div>
        </div>

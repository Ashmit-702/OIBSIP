...
def create_app(config_object: type = Config) -> Flask:
    ...
    # Surface a visible warning in every page when running on Vercel without a
    # real database configured. Without this, SQLite silently falls back to
    # /tmp, which is wiped on every cold start -- data appears to save, then
    # vanishes, with no error anywhere. That's confusing to debug, so make it
    # loud in the UI instead of a footnote in the README.
    from app.models.db import USE_POSTGRES

    @app.context_processor
    def inject_persistence_warning():
        return {
            "db_persistence_warning": config_object.ON_VERCEL and not USE_POSTGRES
        }

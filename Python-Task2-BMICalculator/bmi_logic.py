...
def compute_health_score(bmi: float, category: str, streak: int, history: list,
                          forecast: dict, goal_progress: dict = None) -> dict:
    """
    A transparent, deterministic composite score (0-100) — NOT an AI output,
    and NOT a medical assessment. Every point is traceable to a component
    below, on purpose: a score nobody can explain is worse than no score.

    Components (weights redistribute proportionally if a goal isn't set,
    so the score always totals out of 100):
      - BMI standing   (40): how close BMI sits to the center of the
                              healthy range (~21.7)
      - Consistency    (20): logging streak, reflecting adherence rather
                              than the reading itself
      - Trend direction(20): whether the user's own regression-fit trend
                              (see forecast_bmi_trend) is moving the right
                              way for their category
      - Goal progress  (20): only included when a goal weight is set
    """
    # --- BMI standing (0-40) ---
    center = 21.7
    diff = abs(bmi - center)
    bmi_score = round(max(0.0, min(40.0, 40 - diff * 3)), 1)

    # --- Consistency (0-20) ---
    effective_streak = streak if streak and streak > 0 else (1 if history else 0)
    consistency_score = round(min(20.0, effective_streak * 4), 1)

    # --- Trend direction (0-20) ---
    desired_direction = {
        "Underweight": "rising",
        "Overweight": "falling",
        "Obese": "falling",
    ...
    if not forecast or not forecast.get("available"):
        trend_score = 10.0  # neutral — not enough history to judge yet
    else:
    ...
    components = [
        {"label": "BMI standing", "score": bmi_score, "max": 40},
        {"label": "Consistency", "score": consistency_score, "max": 20},
        {"label": "Trend direction", "score": trend_score, "max": 20},
    ]
    weight_total = 80.0

    if goal_progress and goal_progress.get("percent") is not None:
        goal_score = round(max(0.0, min(100.0, goal_progress["percent"])) / 100 * 20, 1)
        components.append({"label": "Goal progress", "score": goal_score, "max": 20})
        weight_total += 20.0

    raw_sum = sum(c["score"] for c in components)
    final_score = round(raw_sum / weight_total * 100)
